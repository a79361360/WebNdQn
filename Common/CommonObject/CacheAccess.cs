...
namespace FJSZ.OA.Common
{
    ...
    {
        /// <summary>
        /// 设置有绝对过期时间的缓存 会覆盖,绝对过期(即使访问时间也不会延长)
        /// </summary>
        ...
        /// <param name="cacheKey"></param>
        /// <param name="cacheObject"></param>
        /// <param name="seconds">超过多少时间不调用就失效，单位是秒</param>
        public static void InsertToCacheByTimeE(string cacheKey, object cacheObject, int seconds)
        {
            Cache cache = HttpRuntime.Cache;
            cache.Insert(cacheKey, cacheObject, null, DateTime.MaxValue, TimeSpan.FromSeconds(seconds));
        }
        /// <summary>
        /// 添加缓存，不可覆盖
        /// </summary>

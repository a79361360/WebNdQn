...
function ajaxshow(url, title, options, events) {
	...
	html = (typeof title != 'undefined' ? '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3 id="myModalLabel">'+title+'</h3></div>' : '') +
			...
			'<div class="modal-footer">'+(typeof events['confirm'] == 'function' ? '<a href="#" class="btn btn-primary confirm">确定</a>' : '') + '<a href="#" class="my_btn" data-dismiss="modal" aria-hidden="true">关闭</a></div>';
	modalobj.html(html);
...
function kindeditor(selector, callback,maximumWords) {
	UE_SELECTOR.push(selector);	
	if(maximumWords && window.UEDITOR_CONFIG){
		window.UEDITOR_CONFIG.maximumWords = maximumWords;	
	}else if(window.UEDITOR_CONFIG){
		window.UEDITOR_CONFIG.maximumWords = 5000;
	}
	if (UE_LOADING) {
		return false;
	}
...
function UEditor(selector, callback) {
    ...
    if(isBase == true){
        var option = {
            toolbars:[
                ['removeformat', 'bold', 'italic', 'underline', '|', 'forecolor', 'backcolor', 'fontsize', 'paragraph', 'insertorderedlist', 'insertunorderedlist', '|', 'indent', 'justifyleft', 'justifycenter', 'justifyright', 'justifyjustify','|', 'insertimage', 'inserttable', 'deletetable', 'preview',]
            ],
            savePath : ['images'],
            imageFieldName : 'imgFile',
            imageUrl : '../../../../../index.php?act=attachment&do=ueupload',
            imagePath : './resource/attachment/',
            imageManagerUrl : '../../../../../index.php?act=attachment&do=ue_manager',
            imageManagerPath : ''
        }
    }else{
        var option = {
...
function kindeditorUploadBtn(obj, callback, doact) {
	...
	function initUploader() {
		...
		var uploadbutton = KindEditor.uploadbutton({
			button : obj,
			fieldName : 'imgFile',
			url : './index.php?act=attachment&do='+doact,
			width : 100,
